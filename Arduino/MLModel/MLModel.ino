...
const float accelerationThreshold = 0.0; // Threshold (in G values) to detect a "gesture" start
const int numSamples = 250; // Number of samples for a single gesture

int samplesRead; // sample counter 
const int inputLength = 1500; // dimension of input tensor (6 values * 119 samples)
...
void setup() {
  ...
  pinMode(FALLEN_PIN, OUTPUT);
  digitalWrite(FALLEN_PIN, LOW);
  ...
  while (samplesRead < numSamples) {
    ...
    if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
      ...
      if (samplesRead == numSamples) {
        ...
        digitalWrite(FALLEN_PIN, modelGetOutput(1) > modelGetOutput(0) && modelGetOutput(1) > modelGetOutput(2));
      }
    }
  }
}
